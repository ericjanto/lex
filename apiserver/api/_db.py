...
class LexDbIntegrator:
    ...
    def __init__(self, env: DbEnvironment) -> None:
        ...
        self.connection: Client = create_client(
    ...
    def bulk_delete_lemmata(self, lemma_ids: set[LemmaId]) -> bool:
        ...
        """
        lemma_ids_list = list(lemma_ids)
        if not self._all_lemmata_exist(lemma_ids_list):
            return False

        # Get all lemma data before deletion
        lemmata = self.bulk_get_lemmata(lemma_ids_list)
        ...
        for context_id in context_ids:
            ...
            if context_response.data and len(context_response.data) > 0:
                ...
                for lemma_id in lemma_ids_list:
                    updated_context_value = re.sub(
        ...
        self.connection.table("lemma").delete().in_(
            "id", lemma_ids_list
        ).execute()

        # Verify deletion was successful (none of the lemmata should exist
        # anymore)
        return len(self.bulk_get_lemmata(lemma_ids_list)) == 0
