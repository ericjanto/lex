...
class LemmaExtractor:
    ...
    TODO: testing
    TODO: what's the output you *need*?
          Are original line numbers still necessary?
    TODO: currently uses PerceptronTagger, explained here:
                https://explosion.ai/blog/part-of-speech-pos-tagger-in-python
            - Indicates that as a feature, it looks at i-2 and i+2 tags.
              This means that rather than tagging each sentence individually,
              it'll be beneficial to parse sentences in context to
              each other to increase tagging performance near sentence
              boundaries.
    ...
    def extract_lemmata(self) -> list[LemmatisedSentence]:
        ...
        for line_num, sentence in enumerate(self.text):
            ...
            if tokens := word_tokenize(sentence, language=self.language_full):
                # Stopword removal
                tokens = list(
                    filter(lambda x: x not in self.stopwords, tokens)
                )
                ...
                relevant_tokens = list(
                    filter(
                        lambda x: x.tag in relevant_universal, tagged_tokens
                    )
                )
                ...
                lemmatised_sentences.append(
                    LemmatisedSentence(line_num, tagged_lemmata)
                )
