...
serve(async (req) => {
    ...
    try {
        ...
        // Only set schema if it is exposed. lex_mock is NOT exposed.
        // UPDATE: User confirmed lex_mock IS exposed.
        const clientSchema = targetSchema

        // Use service role key for DB operations (bypasses RLS)
        const supabaseClient = createClient(
            Deno.env.get('SUPABASE_URL') ?? '',
            Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '',
            { db: { schema: clientSchema } }
        )
        ...
        if (path === '/' || path === '/api_status') {
            // Debugging key prefixes
            const anonKey = Deno.env.get('SUPABASE_ANON_KEY') ?? ''
            const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''
            return new Response(JSON.stringify({
                api_status: 'working',
                debug_anon_key_prefix: anonKey.substring(0, 10),
                debug_service_key_prefix: serviceKey.substring(0, 10)
            }), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            })
        }
        ...
        if (path === '/lemma' && method === 'DELETE') {
            ...
            for (const id of lemma_ids) {
                try {
                    ...
                    const contextIds = (links || []).map((l: any) => l.context_id)
                    ...
                    for (const cid of contextIds) {
                        ...
                        const distinctLemmataIds = new Set((allLinks || []).map((l: any) => l.lemma_id))
        ...
        if (path === '/status_lemmata' && method === 'GET') {
            ...
            if (statusError || !statusData) {
                const { data: allStatuses } = await supabaseClient.from('lemma_status').select('status')
        ...
        if (path.startsWith('/source_contexts/') && method === 'GET') {
            const source_id = path.replace('/source_contexts/', '')
            const page = parseInt(url.searchParams.get('page') ?? '1')
            const page_size = parseInt(url.searchParams.get('page_size') ?? '100')
            const start = (page - 1) * page_size
            const end = start + page_size - 1
            ...
            const { data, error } = await supabaseClient
                .from('context')
                .select('*, lemmata:lemma_context(lemma(id, lemma))')
